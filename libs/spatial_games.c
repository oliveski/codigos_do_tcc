...
void initMemory(Memory *mem, Lattice net, Params p){
	...
	for(int i = 0; i < p.L2; i++){
		...
		for(int j = 1; j < p.M; j++){
			...
			mem->hist[i][j] = net.players[i];
#ifdef GOOD
			// my history is my strategy at t=0
			// and for t<0 it is as if I had cooperated
			// this is good faith memory
			mem->hist[i][j] = 1;
#endif
#ifdef EVIL
			// my history is my strategy at t=0
			// and for t<0 it is as if I had defected
			// this is man is evil memory
			mem->hist[i][j] = 0;
#endif
#ifdef RNDMEM
			// my history is my strategy at t=0
