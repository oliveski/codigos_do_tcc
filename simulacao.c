...
int main(int argc, char *argv[]){
	...
	for(int mcs = 1; mcs <= MCS; mcs++){
		...
		/* 	sprintf(snpsht, "snpsht1_r%.4lf_L%d_S%d_M%d_tau%.4f.eps", */
		...
		/* if(mcs == 1000){ */
		/* 	lpp = linksPerPlayer(net); */
		/* 	sprintf(snpsht, "snpsht2_r%.4lf_L%d_S%d_M%d_tau%.4f.eps", */
		/* 			p.r, p.L, p.seed, p.M, p.tau); */
		/* 	openlat(p.L, 4); */
		/* 	printlat(net, hks, lpp, snpsht, 4); */
	...
	sprintf(histname, "clstr_hstgrm_r%.4lf_L%d_S%d_M%d_tau%.4f.dsf",
			p.r, p.L, p.seed, p.M, p.tau);
	sprintf(percolname, "prcltn_1_r%.4lf_L%d_S%d_M%d_tau%.4f.dsf",
			p.r, p.L, p.seed, p.M, p.tau);
	// state 1
	...
	// bond
	sprintf(histname, "bond_clstr_hstgrm_r%.4lf_L%d_S%d_M%d_tau%.4f.dsf",
			p.r, p.L, p.seed, p.M, p.tau);
	sprintf(percolname, "bond_prcltn_r%.4lf_L%d_S%d_M%d_tau%.4f.dsf",
			p.r, p.L, p.seed, p.M, p.tau);
	initHK(&hks, p.L, histname, percolname);
	bond_hk(net, &hks);
	percolation_measures(&hks);
